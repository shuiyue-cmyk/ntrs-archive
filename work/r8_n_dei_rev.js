// R8 NTRS extension fix for Cirno_NTRS_turn_edit_DEI_revise_4.7.json (DEI revise variant)
// Pairs are [old, new] JS template literals. Copies OLD byte-for-byte from spec N-extension,
// adjusted for revise variant: 3-space indent in HARD RULES list, 5-space indent on N-B3 line.
const fs = require('fs');
...
const pairs = [
  ...
  // ---------- N-A2: HARD RULES 3「出场可能性判定」整段（revise 3 空格缩进） ----------
  [
  ...
  // ---------- N-B3: S3-MSG2 prologue 黄毛登场门 no_spawn 行（revise 5 空格缩进） ----------
  [
  ...
  // ---------- N-C1: S2-MSG0「黄毛行动不依赖本轮是否刷新在场」句尾补（revise 版内嵌句） ----------
  [
...
// collect all editable strings
const fields = [];
for (const t of p.plotTasks) {
  fields.push({ task: t.id, field: 'description', s: t.description });
  t.promptGroup.forEach((m, i) => {
    if (typeof m.content === 'string') fields.push({ task: t.id, field: 'MSG' + i, s: m.content });
  });
}
fields.push({ task: 'FSD', field: 'finalSystemDirective', s: p.finalSystemDirective });

const report = [];
pairs.forEach(([oldS, newS], idx) => {
  let total = 0;
  let anyFail = false;
  const nid = 'N-' + ['A1', 'A2', 'A3', 'A4', 'A6', 'B1', 'B2', 'B3', 'C1', 'C2'][idx];
  const foundIn = [];
  for (const f of fields) {
    const cnt = f.s.split(oldS).length - 1;
    if (cnt > 0) {
      total += cnt;
      foundIn.push(f.task + ':' + f.field);
      f.s = f.s.split(oldS).join(newS);
    }
  }
  if (total === 0) anyFail = true;
  report.push({ id: nid, count: total, ok: !anyFail, in: foundIn.join(', ') });
});

// write fields back
for (const t of p.plotTasks) {
  t.description = fields.find(f => f.task === t.id && f.field === 'description').s;
  t.promptGroup.forEach((m, i) => {
    const f = fields.find(x => x.task === t.id && x.field === 'MSG' + i);
    if (f) m.content = f.s;
  });
}
p.finalSystemDirective = fields.find(f => f.task === 'FSD').s;

const outRaw = JSON.stringify(j, null, 2);
// safety: must parse AND start with '['
try {
  JSON.parse(outRaw);
} catch (e) {
  console.log('FATAL: output does not parse, not writing');
  process.exit(1);
}
if (!outRaw.trim().startsWith('[')) {
  console.log('FATAL: output top level not array, not writing');
  process.exit(1);
}
fs.writeFileSync(path, outRaw, 'utf8');

console.log('=== HIT COUNTS ===');
for (const r of report) console.log(r.id + ': ' + r.count + (r.ok ? ' OK' : ' FAIL (0 hits)') + (r.in ? '  @ ' + r.in : ''));
