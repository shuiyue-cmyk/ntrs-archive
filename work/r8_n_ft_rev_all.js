...
const pairs = [
  ...
  // N-A2: HARD RULES 3「出场可能性判定」整段 (revise 版 3 空格缩进)
  ...
  // N-B2: S3 stage 模板「场景外标注」字段 (本文件用「对象」措辞)
  ...
  // N-B3: S3-MSG2 prologue 黄毛登场门 no_spawn 行 (ALLin 措辞)
  ...
  // N-C1 (ALLin, 该黄毛措辞): S2-MSG0「黄毛行动不依赖本轮是否刷新在场」句尾补
  ['**黄毛行动不依赖本轮是否刷新在场**——只要该黄毛离场攻略目标（尾随/赶赴/在 {{user}} 场景外接近目标）在剧情上合理，即使本轮 no_spawn、目标与黄毛均不在 {{user}} 当前场景，也可判 act，该行动发生在 {{user}} 场景外）',
   '**黄毛行动不依赖本轮是否刷新在场**——只要该黄毛离场攻略目标（尾随/赶赴/在 {{user}} 场景外接近目标）在剧情上合理，即使本轮 no_spawn、目标与黄毛均不在 {{user}} 当前场景，也可判 act，该行动发生在 {{user}} 场景外）。**目标与 {{user}} 同处当前场景时（该黄毛已真正锁定）**：该黄毛虽不在 {{user}} 当前场景画面内（no_spawn），但可主动制造目标离开 {{user}} 场景的机会（约定/诱引/传递信息/外部事件引走目标等合理手段）后，在 {{user}} 场景外对目标展开行动——场景外行动照常判 act。**未锁定（背景板）黄毛仍一律 no-act，不适用本条**。'],

  // N-C2 (optional): S2-MSG4 行动判定 no-act 定义句
...
const ids = ['N-A1', 'N-A2', 'N-A3', 'N-A5', 'N-A6', 'N-B1', 'N-B2', 'N-B3', 'N-C1', 'N-C2'];

// build target list with proper owner references
const targets = [];
for (const t of j[0].plotTasks) {
  targets.push({ owner: t, kind: 'description' });
  for (const m of t.promptGroup || []) targets.push({ owner: m, kind: 'content' });
}
targets.push({ owner: j[0], kind: 'fsd' });

const apply = (tgt, old, next) => {
  if (tgt.kind === 'description') tgt.owner.description = tgt.owner.description.split(old).join(next);
  else if (tgt.kind === 'content') tgt.owner.content = tgt.owner.content.split(old).join(next);
  else tgt.owner.finalSystemDirective = tgt.owner.finalSystemDirective.split(old).join(next);
};

let allOk = true;
const report = [];
pairs.forEach(([old, next], i) => {
  let count = 0;
  for (const tgt of targets) {
    const s = tgt.kind === 'description' ? tgt.owner.description
      : tgt.kind === 'content' ? tgt.owner.content
      : tgt.owner.finalSystemDirective;
    if (s.includes(old)) {
      count += s.split(old).length - 1;
      apply(tgt, old, next);
    }
  }
  const ok = count > 0;
  if (!ok) allOk = false;
  report.push({ id: ids[i], count, ok });
});

const out = JSON.stringify(j, null, 2);
if (JSON.parse(out) && raw.trim().startsWith('[')) {
  fs.writeFileSync(path, out, 'utf8');
  console.log('WROTE file. bytes:', out.length);
} else {
  console.log('ABORT write: parse/array check failed');
  allOk = false;
}
for (const r of report) console.log(`${r.id}: hit=${r.count} ${r.ok ? 'OK' : 'FAIL'}`);
