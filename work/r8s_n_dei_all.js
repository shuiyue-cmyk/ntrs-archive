// R8s fix (v2): G1/G2/G4 on Cirno_NTRS_turn_edit_DEI_ALLin_4.7.json — mutate object fields in place
const fs = require('fs');
...
if (!raw.trimStart().startsWith('[')) throw new Error('raw does not start with [ - abort');
const j = JSON.parse(raw);

// collect [owner, key] field references
const refs = [];
for (const t of j[0].plotTasks || []) {
  for (const msg of t.promptGroup || []) if (typeof msg.content === 'string') refs.push([msg, 'content']);
  if (typeof t.description === 'string') refs.push([t, 'description']);
}
if (typeof j[0].finalSystemDirective === 'string') refs.push([j[0], 'finalSystemDirective']);

const counts = {};
for (const [name, old] of pairs) {
  counts[name] = refs.reduce((acc, [o, k]) => acc + o[k].split(old).length - 1, 0);
}
console.log('hit counts:', JSON.stringify(counts));

for (const [name, old, nw] of pairs) {
  for (const [o, k] of refs) o[k] = o[k].split(old).join(nw);
}

const out = JSON.stringify(j, null, 2);
JSON.parse(out);
fs.writeFileSync(p, out, 'utf8');
console.log('written OK, starts with [:', out.trimStart().startsWith('['));
