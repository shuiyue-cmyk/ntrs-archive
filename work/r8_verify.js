...
for (const [id, old, next] of pairs) {
  const oldPresent = blob.includes(old);
  const newPresent = blob.includes(next);
  if (oldPresent || !newPresent) allOk = false;
  console.log(id + ' oldGone=' + (!oldPresent) + ' newPresent=' + newPresent + (oldPresent ? '  <-- OLD STILL PRESENT' : '') + (newPresent ? '' : '  <-- NEW MISSING'));
}
for (const f of ['接下来的场景中是否会有黄毛出现的可能', '后续剧情是否有黄毛实际出场的契机', 'prologue 不展开该场景外戏']) {
  console.log('residual[' + f + ']=' + blob.includes(f));
}
