...
if (!Array.isArray(j)) { console.log('FATAL: top-level not array before edit'); process.exit(1); }
const p = j[0];

// [old, new] pairs — ALLin variant
const pairs = [
  ...
  // N-A2: HARD RULES 3「出场可能性判定」整段 (1-space indent, ALLin)
  [
 ...
 - 若接下来的场景中黄毛有合理出场路径（目标将去公共场所、黄毛可被引荐/偶遇/主动接近、后续互动有展开空间）→ 判 **spawn**`,
  ...
  // N-B3: S3-MSG2 prologue 黄毛登场门 no_spawn 行
  [
  ...
  // N-C1: S2-MSG0「黄毛行动不依赖本轮是否刷新在场」句尾 (ALLin embedded, 该黄毛)
  [
    `也可判 act，该行动发生在 {{user}} 场景外）`,
    `也可判 act，该行动发生在 {{user}} 场景外。**目标与 {{user}} 同处当前场景时（黄毛已真正锁定）**：黄毛虽不在 {{user}} 当前场景画面内（no_spawn），但可主动制造目标离开 {{user}} 场景的机会（约定/诱引/传递信息/外部事件引走目标等合理手段）后，在 {{user}} 场景外对目标展开行动——场景外行动照常判 act。**未锁定（背景板）黄毛仍一律 no-act，不适用本条**）`,
  ],
...
// collect all text fields to replace in
const texts = [];
for (const t of p.plotTasks) {
  texts.push({ where: t.id + '|description', s: t.description });
  if (Array.isArray(t.promptGroup)) {
    for (let i = 0; i < t.promptGroup.length; i++) {
      texts.push({ where: t.id + '|promptGroup[' + i + ']', s: t.promptGroup[i].content });
    }
  }
}
...
for (let pi = 0; pi < pairs.length; pi++) {
  const [oldS, newS] = pairs[pi];
  let total = 0;
  for (const t of texts) {
    if (t.s.includes(oldS)) {
      const n = t.s.split(oldS).length - 1;
      total += n;
      t.s = t.s.split(oldS).join(newS);
    }
  }
  const ok = total > 0;
  if (!ok) anyFail = true;
  results.push({ id: 'N-' + (pi + 1), count: total, ok });
  console.log(`pair ${pi + 1}: hits=${total} ${ok ? 'OK' : 'FAIL'}`);
}

// write back texts
let ti = 0;
for (const t of p.plotTasks) {
  t.description = texts[ti++].s;
  if (Array.isArray(t.promptGroup)) {
    for (let i = 0; i < t.promptGroup.length; i++) {
      t.promptGroup[i].content = texts[ti++].s;
    }
  }
}
p.finalSystemDirective = texts[ti].s;
...
if (out.trim().startsWith('[')) {
  ...
  console.log('WRITTEN. anyFail=' + anyFail);
  for (const r of results) console.log(JSON.stringify(r));
} else {
  console.log('FATAL: output not top-level array, NOT written');
  process.exit(1);
}
