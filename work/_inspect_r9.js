// R9 inspect: locate PART B OLD strings in Cirno_NTRS_turn_edit_straight_revise_4.7.json
const fs = require('fs');
const path = 'C:/Users/zouyu/Downloads/酒馆/数据库/剧情推进预设/Cirno_NTRS_turn_edit_straight_revise_4.7.json';
const raw = fs.readFileSync(path, 'utf8');
let j;
try { j = JSON.parse(raw); } catch (e) { console.log('JSON PARSE FAIL', e.message); process.exit(1); }
console.log('top starts with [', raw.trim().startsWith('['));
const p = j[0];
console.log('name:', p.name);

// collect all text-bearing fields
const texts = [];
for (const t of p.plotTasks || []) {
  texts.push({ where: `task[${t.id}].description`, s: t.description || '' });
  (t.promptGroup || []).forEach((m, i) => texts.push({ where: `task[${t.id}].promptGroup[${i}]`, s: m.content || '' }));
}
texts.push({ where: 'finalSystemDirective', s: p.finalSystemDirective || '' });

const blob = texts.map(x => x.s).join('\n=====\n');

const pats = [
  ['B1', 'prologue：仅一行「跟随{{user}}输入的主线走，本轮黄毛不出手，剧情按输入自然推进」'],
  ['B2', '属 📹 事后知情或 🌙 完全不知的暗线戏'],
  ['B3', '锁定指令：锁定 / 维持背景板'],
  ['B3b', '标签内只放刷新状态'],
  ['B4a', '上轮阶段名+上轮%'],
  ['B4b', '上轮%'],
  ['B5', '判断该已有黄毛本轮是否可行动'],
  ['B5b', '判断该黄毛本轮是否可行动'],
  ['B8', 'locked_target 命中本轮登场名单里某💔敏感角色名'],
  ['B8b', 'locked_target 命中'],
  ['B10', '锁定状态'],
];
for (const [tag, s] of pats) {
  const count = blob.split(s).length - 1;
  console.log(`\n[${tag}] count=${count} pat=${JSON.stringify(s)}`);
  if (count > 0) {
    // show first occurrence context
    const idx = blob.indexOf(s);
    console.log('  ctx: ...' + JSON.stringify(blob.slice(Math.max(0, idx - 120), idx + s.length + 80)) + '...');
  }
}
